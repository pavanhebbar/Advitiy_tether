...
def getB_NED(lat, longit, r):
    B_x, B_y, B_z, B_t = igrf12.igrf12syn(0, 2017, 2, r*10**-3, (np.pi/2 - lat)*180/np.pi, longit*180/np.pi)
...
class sat_param(orbit_state):
    ...
    def accln(self):
        ...
        if (self.b_B == 1):
            B_r, B_t, B_p = self.getB()
            a_t = -1*self.getcurr()*self.leng*B_p/self.mass
            a_p = self.getcurr()*self.leng*B_t/self.mass
        elif (self.b_B == 0):
            a_t = 0
            a_p = 0
        return a_r, a_t, a_p
    ...
    def power_dis(self):
        a_r, a_t, a_p = self.accln()
        v_r, v_t, v_p = self.getvel_sph()
        return (v_t*a_t + v_p*a_p)*self.mass

    def get_en(self):
        v_r, v_t, v_p = self.getvel_sph()
        r, theta, phi = self.get_pos()
        return 0.5*self.mass*(v_r**2 + v_t**2 + v_p**2) - G*M*self.mass/r
...
def orbit(sat, tfinal, tstep):
    ...
    curr = np.zeros(n_t)
    en_arr = np.zeros(n_t)
    pow1_arr = np.zeros(n_t)
    pow2_arr = np.zeros(n_t)
    en0 = sat.get_en()
    count = 0
    for i in range(ntimes):
        ...
        if (i % (int)(100/tstep) == 0):
            ...
            curr[count] = sat.getcurr()
            lat_arr[count], long_arr[count] = sat.getlatlong()
            acc_arr[count, :] = sat.accln()
            pow1_arr[count] = sat.power_dis()
            pow2_arr[count] = pow2
            en_arr[count] = sat.get_en()
            count += 1
            print (count)

    return r_array, theta_array, phi_array, lat_arr, long_arr, curr, vr_arr, vt_arr, vp_arr, acc_arr, pow1_arr, pow2_arr, en_arr
